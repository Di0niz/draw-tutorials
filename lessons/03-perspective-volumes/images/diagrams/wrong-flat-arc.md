...
`wrong-flat-arc.svg` (как не надо), справа `right-wrapping-arc.svg` (как надо).
...
**Статус:** `wrong-flat-arc.svg` и `right-wrapping-arc.svg` — временные векторные
заглушки, подлежат замене парой карандашных пластин в стиле
